...
Options:
  -m, --multiprocess     Run multiprocess binaries tesser-node, tesser-gui.
  -M, --monolithic       Run monolithic binaries tesserd, tesser-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'tesser-qt [ARGS]' or 'tesser-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'tesserd [ARGS]' or 'tesser-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'tesser-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'tesser-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'tesser-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_tesser [ARGS]'.
  chainstate [ARGS] Run tesser kernel chainstate util, equivalent to running 'tesser-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_tesser [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_tesser-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "tesser-gui" : "tesser-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "tesser-node" : "tesserd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("tesser-cli");
            // Since "tesser rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("tesser-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("tesser-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_tesser");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("tesser-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_tesser");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_tesser-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("tesser-util");
        } else {
...
//! Execute the specified tesserd, tesser-qt or other command line in `args`
...
//!             be a relative path to a tesserd, tesser-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/tesser ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/tesser/tesser/pull/31375#discussion_r1861814807)
